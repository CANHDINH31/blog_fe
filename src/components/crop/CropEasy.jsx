...
const CropEasy = ({ photo, setOpenCrop }) => {
  ...
  const handleCropImage = async () => {
    try {
      ...
      const img = await uploadImgToFirebase(file);

      mutate({ token: userState.userInfo.token, formData: { avatar: img } });
    } catch (error) {
      toast.error(error.message);
      console.log(error);
    }
    ...
    <div className="fixed inset-0 z-[1000] flex justify-center overflow-auto bg-black/50 p-5">
      <div className="h-fit w-full rounded-lg bg-white p-5 sm:max-w-[350px]">
        ...
        <div>
          <label
            htmlFor="zoomRage"
            className="mt-2 mb-0.5 block text-sm font-medium text-gray-900"
          >
